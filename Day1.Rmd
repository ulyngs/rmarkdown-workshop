...
.col-4.beginner[
In a new R Markdown document, use code chunks to
- include an image with `knitr::include_graphics`
- include a plot (e.g. `plot(pressure)`)
- include a table (using `kable`)
- report a calculation inline
]
  
.col-4.intermediate[
- try resizing plots with `out.width` and `fig.width` - what's the difference?
- how do you set `knitr`'s global options to hide code by default? 
- what other options are available to control if a code block is executed and what results are inserted in the finished report? (Hint: see [R for Data Science, 27.4.2](https://r4ds.had.co.nz/r-markdown.html))
]

.col-4.advanced[
- what's the use of `cache = TRUE`? How does it relate to the `dependson` chunk option?
- how might you create new chunk options, if the ones provided by `knitr` are not sufficient?
]
]
...
<ol>
<li> Put references in a plain text file with the extension **.bib**, in **BibTex** format (most reference managers can do this - [Zotero](https://www.zotero.org) works best, I use [Mendeley](https://www.mendeley.com)).<sup>1</sup> <br>
Note that 'Shea2014' in the highlighted section is the **citation identifier**.
</ol>
```bibtex
*@article{Shea2014,
...
Shea et al. (2014) says blah.<br>
Shea et al. (2014, 33) says blah.
...
## Section references
Whenever we use a header (with a \#), a label is automatically assigned, namely the header name in lower-caps separated by dashes: <br> **So a section `# My section` is automatically given the label `my-section`.**

To make a cross-reference, use the syntax `\@ref(label)`. <br> **So we can refer to that section above with `\@ref(my-section)`.**

You can add your own labels to headers by adding `{\#label}` after a header, e.g. `\#My section {\#my-label}`.

See examples here: [Rmd file](examples/cross_referencing.Rmd) and [HTML output](examples/cross_referencing.html).
...
--
.pull-right[
- Recall that the first chunk option after the language is the label (we could also be explicit with `label=captain`).
- To refer to figures (plots and images) use the syntax `\@ref(fig:label)`
- So we'd refer to this image with `\@ref(fig:captain)`
- See examples here: [Rmd](examples/cross_referencing.Rmd), [HTML](examples/cross-referencing.html)
]
...
- So we'd refer to this image with `\@ref(tab:cars)`
- See examples here: [Rmd](examples/cross_referencing.Rmd), [HTML](examples/cross-referencing.html)
]
---
...
2. Open **examples/citation_test.Rmd** and add a new paragraph with two citations - what happens when you knit?
3. Make cross-references to a figure and a table
]
  
.col-4.intermediate[
1. Add a custom label to a header and reference it
2. Have a look through [zotero.org/styles](https://www.zotero.org/styles); download two new styles and try them out in **examples/citation_test.Rmd**.

]

.col-4.advanced[
1. How would you handle a situation where special characters made `fig.cap` mess up? (.black[[hint](https://bookdown.org/yihui/bookdown/markdown-extensions-by-bookdown.html#text-references)])
